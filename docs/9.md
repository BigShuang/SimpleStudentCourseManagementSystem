...
由于课程会有多个状态，这个可以先在`constants.py`中去记录好（即添加代码如下）
```python
COURSE_STATUS = {
    1: "未开始选课",
    2: "开始选课",
...
补充说明： `get_status_text`和`get_op_text`方法是为了方便在模板中调用。
...
同[第三节第三部分](https://github.com/BigShuang/SimpleStudentCourseManagementSystem/blob/master/docs/3.md)
